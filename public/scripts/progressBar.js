...
for (let i = 0; i < category.length; i++) {
    ...
    progressBar[i].setAttribute('style', `background-image: linear-gradient(90deg, #198754 ${w}%, white ${w}%)!important`) // progress bar color if less than 100%
    if (w >= 100) {
        progressBar[i].style.width = `100%`
        progressBar[i].setAttribute('style', `background-image: linear-gradient(90deg, #DC1C13 0%, #DC1C13 100%)!important`) // progress bar color if 100% or more
    }
}